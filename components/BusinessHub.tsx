...
import { ShieldCheck, Download, Calendar, Loader2, Sparkles } from 'lucide-react';
...
const BusinessHub: React.FC<BusinessHubProps> = ({ lang, context }) => {
  ...
  const [arrivalInfo, setArrivalInfo] = useState(lang === 'cn' ? `到达 ${context.to}，日期 ${context.date}` : `Arrive in ${context.to} on ${context.date}`);
  const [meetingSuggestion, setMeetingSuggestion] = useState('');
  const [showReport, setShowReport] = useState(false);
  const [reportSummary, setReportSummary] = useState('');
  const [reportLoading, setReportLoading] = useState(false);

  // Sync with global trip data
  useEffect(() => {
    setArrivalInfo(lang === 'cn' ? `到达 ${context.to}，日期 ${context.date}` : `Arrive in ${context.to} on ${context.date}`);
  }, [context, lang]);
    ...
    if (!showReport) {
      ...
      try {
        const tripSummary = {
          ...
          project: `${context.to} Q2 Expansion`,
          total: 9700,
          compliance: '92%'
        };
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <button 
             ...
             >
               {reportLoading ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />} {showReport ? "Close Report" : "Generate AI Report"}
            </button>
          </div>
          {showReport && reportSummary && (
            <div className="mt-4 p-6 bg-slate-50 border border-slate-100 rounded-2xl animate-in slide-in-from-top-2">
              <div className="flex items-center gap-2 mb-3 text-indigo-600">
                <Sparkles size={18} />
                <h4 className="font-bold text-sm">AI Executive Summary for {context.to}</h4>
              </div>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            ...
            <div className="space-y-4">
              <div className="text-xs font-bold text-indigo-200">{lang === 'cn' ? '到达信息：' : 'Arrival Info:'} {arrivalInfo}</div>
              <textarea 
                value={meetingInput}
          ...
          <div className="bg-white/10 border border-white/20 rounded-2xl p-6 min-h-[200px] overflow-y-auto">
             {meetingSuggestion ? (
               ...
               <div className="h-full flex flex-col items-center justify-center text-indigo-200 text-center py-8">
                 <p className="text-xs">Provide details to generate your optimized business schedule for {context.to}.</p>
               </div>
             )}
          </div>
        </div>
